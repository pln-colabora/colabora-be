...
### API docs (Scalar)
`GET /docs` serves a [Scalar](https://scalar.com/) API Reference UI (registered in `docs/routes.go`, wired in `cmd/main.go` alongside module routes). The self-contained documents are `health.yaml`, `auth.yaml`, `user.yaml`, seven `stage-*.yaml` workflow documents for permohonan, and `document.yaml`. These files describe endpoints that actually exist; future target contracts belong in `API_SPEC.md` until implemented. Register every new module or workflow-stage document in both `docs/routes.go` and `docs/openapi_merge.go`. The Scalar CDN script is version-pinned with matching SRI; update URL and hash together.
