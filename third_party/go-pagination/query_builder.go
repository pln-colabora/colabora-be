...
func PaginatedQueryWithOptions[T any](
	...
	// LOCAL PATCH (colabora-be): upstream v0.1.0 never applied the ?search=
	// term to the count query, only to dataQuery below — total/max_page were
	// wrong whenever Search was set. Mirror dataQuery's search handling here.
	if pagination.Search != "" {
		countQuery = applyAutoSearch(countQuery, pagination.Search, builder.GetSearchFields(), options.Dialect)
	}
