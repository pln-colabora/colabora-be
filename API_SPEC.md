...
Vendor WO and execution payloads may optionally include `location_coordinates` as an object with decimal WGS84 `latitude` (`-90..90`) and `longitude` (`-180..180`). Both values are required when the object is present. The field is currently stored in the workflow-node JSONB payload for all vendor submissions and remains optional while vendor forms are temporary.
