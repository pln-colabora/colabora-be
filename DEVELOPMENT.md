...
2. **Complete:** Parallel WO Tiang/Konstruksi/APP, vendor-konstruksi material reservation, transaksi-energi tera, and conditional WO PDKB. The former PK Vendor gate has since been retired.
...
Sequence 2 delivers the six Stage 4 routes for the three parallel WO branches, independent material reservation by the explicitly assigned `vendor-konstruksi` account, independent APP tera by `transaksi-energi`, and conditional WO PDKB. WO Konstruksi records the typed `perlu_pdkb` decision: the false branch audits and persists both PDKB skips, while the true branch gates construction execution on WO PDKB. The former PK Vendor gate is retired; historical rows and evidence remain available for audit. Tests cover both connection families, parallel completion in opposite orders, both pole/PDKB branches, exact owners, terminal/out-of-order/duplicate rejection, per-node evidence rollback, and audit events for derived skips. Payloads remain intentionally minimal where production fields are still discovery gates.
